...
dms_archive_dir = "C:/Users/Claire/Box/CKouba_Dissertation_DMS"
if(!dir.exists(dms_archive_dir)){dms_archive_dir = "C:/Users/cmkouba/Box/CKouba_Dissertation_DMS"}
# latest_vmp_wl_data_filename = "ScottValleyGWMonitProg_2022-11_confidential.csv"

svihm_scenarios_dir = NA
read_in_model_results = FALSE

#_SPATIAL DATA ------------------------------------------------------------

# if(retrieve_spatial_data==TRUE)
...
ca_or = st_transform(x = st_as_sf(ca_or), crs = st_crs(3310))
...
# counties_all = readOGR( file.path(scratch_dir, "California_Counties.shp"))
...
# unzip(file.path(scratch_dir, zipname), exdir = file.path(scratch_dir)) #, list = TRUE) # just lists files, does not unzip
# #Read shapefile into R
# par_all = readOGR(file.path(scratch_dir, "SiskiyouParcelsNov2017.shp"))
# par = spTransform(par_all, crs("+init=epsg:3310"))
#
# #Copy to the box DMS archive
...
# if(!file.exists(file.path(dms_archive_dir, "parcels.shp"))){
#   writeOGR(obj = par, dsn = dms_archive_dir, layer = "parcels", driver = "ESRI Shapefile")
# }
# #remove files from scratch drive
# file.remove(file.path(scratch_dir, zipname))
# extension_list = c("cpg", "dbf", "prj", "shx", "shp", "xml", "sbn", "sbx", "shp.xml")
...
# census_tracts_ca_url = "https://www2.census.gov/geo/tiger/TIGER2021/TRACT/tl_2021_06_tract.zip"
# zipname = strsplit(census_tracts_ca_url, "/")[[1]][length(strsplit(census_tracts_ca_url, "/")[[1]])]
# tracts_dl = GET(census_tracts_ca_url, write_disk(file.path(scratch_dir,zipname), overwrite = TRUE))
# # Unzip file and save in the working directory (defaults to Documents folder)
# unzip(file.path(scratch_dir, zipname), exdir = file.path(scratch_dir))
# #Read shapefiles into R
# tracts_all = readOGR( file.path(scratch_dir, "tl_2021_06_tract.shp"))
# tracts_all = spTransform(tracts_all, crs("+init=epsg:3310"))
# # subset by county
# tracts = tracts_all[county,]
# # find census ids that overlap the watershed (requires loading data first)
# # wsh_tracts = tracts[watershed,]
# # plot(wsh_tracts)
# # plot(watershed, add=T, border = "blue", lwd = 3)
# # plot(wsh_tracts, add=T, border = "red", lwd = 2)
# # plot(gCentroid(wsh_tracts, byid=T), add=T)
# # pointLabel(x = gCentroid(wsh_tracts, byid=T)@coords, labels = wsh_tracts$NAMELSAD, col = "red")
# # big overlap: 8, 6. small overlap: 13, 7.01. exclude: 1.02, 9.01
#
# # #Copy to the box DMS archive
# writeOGR(obj = tracts, dsn = dms_archive_dir, layer = "tiger_census_tracts_siskiyou_county", driver = "ESRI Shapefile")
# #remove files from scratch drive
# file.remove(file.path(scratch_dir, zipname))
# extension_list = c("cpg", "dbf", "prj", "shx", "shp")
# file.remove(file.path(scratch_dir,paste("tl_2021_06_tract", extension_list, sep = ".")))


# Census blocks (TIGER) ---------------------------------------------------

# census_blocks_ca_url = "https://www2.census.gov/geo/tiger/TIGER2021/TABBLOCK20/tl_2021_06_tabblock20.zip"
# zipname = strsplit(census_tracts_ca_url, "/")[[1]][length(strsplit(census_tracts_ca_url, "/")[[1]])]
# blocks_dl = GET(census_blocks_ca_url, write_disk(file.path(scratch_dir,zipname), overwrite = TRUE))
# # Unzip file and save in the working directory (defaults to Documents folder)
# unzip(file.path(scratch_dir, zipname), exdir = file.path(scratch_dir))
# #Read shapefiles into R
# blocks_all = readOGR( file.path(scratch_dir, "tl_2021_06_tabblock20.shp"))
# blocks_all = spTransform(blocks_all, crs("+init=epsg:3310"))
# # find census ids that overlap the watershed (requires loading watershed layer first)
# wsh_blocks = blocks_all[watershed,]
# plot(wsh_blocks)
# # wsh_blocks@data
# plot(watershed, add=T, border = "blue", lwd = 3)
# # plot(wsh_blocks, add=T, border = "red", lwd = 2)
#
# # #Copy to the box DMS archive
# writeOGR(obj = wsh_blocks, dsn = dms_archive_dir, layer = "tiger_census_blocks_scott_watershed", driver = "ESRI Shapefile")
# #remove files from scratch drive
# file.remove(file.path(scratch_dir, zipname))
# extension_list = c("cpg", "dbf", "prj", "shx", "shp")
# file.remove(file.path(scratch_dir,paste("tl_2021_06_tabblock20", extension_list, sep = ".")))
...
# unzip(file.path(scratch_dir,zipname), exdir = file.path(scratch_dir))#, list = TRUE)
# # Read shapefile
# basins_1801_all = readOGR(file.path(scratch_dir, "Shape","WBDHU6.shp"))
# #subset the Klamath basin and reproject to 3310
# klamath = basins_1801_all[basins_1801_all$huc6==kb_huc6,]
# klamath = spTransform(klamath, crs("+init=epsg:3310"))
# # Write to Dropbox
# if(!file.exists(file.path(dms_archive_dir, "klamath_basin_boundary_2018.shp"))){
#   writeOGR(obj = klamath, dsn = dms_archive_dir, layer = "klamath_basin_boundary_2018", driver = "ESRI Shapefile")
# }
# #Remove scratch work
# file.remove(file.path(scratch_dir, zipname))
# file.remove(file.path(scratch_dir, "Shape", list.files(file.path(scratch_dir, "Shape")))) #deletes files in unzipped "Shape" folder
...
# unzip(file.path(scratch_dir, zipname), exdir = file.path(scratch_dir))#, list = TRUE) # just lists files, does not unzip
# #Read shapefile into R
# tribal_all = readOGR(file.path(scratch_dir, "tl_2021_us_aiannh.shp"))
# tribal_all = spTransform(tribal_all, crs("+init=epsg:3310"))
# # county = get_postgis_query(siskiyou_spatial, "SELECT * FROM siskiyou_county_boundary", geom_name = "geom")
# # read in klamath boundary. requires klamath loading (later in this script)
# klamath = readOGR(file.path(dms_archive_dir, "klamath_basin_boundary_2018.shp"))
# tribal = tribal_all[klamath,] # clip to klamath
#
# #Copy to the box DMS archive
# writeOGR(obj = tribal, dsn = dms_archive_dir, layer = "am_indian_areas_klamath", driver = "ESRI Shapefile", overwrite=T)
# #Write to database
# #remove files from scratch drive
...
# # requires loading county spatial layer
#
# #TO DO: match irrigation perforation info with perf table from DWR data
# #TO DO: see if any DWR, VMP wells are co-located (and might be the same well)
#
...
#
#   if(!is.element(el = "ID", colnames(data))){break} # Dozens of these tables have no ID column and no way to subset
#
...
# }
#
# get_wl_data = function(VMP = TRUE, DWR = TRUE,  PSI = FALSE,
#                        LWA_TD = FALSE, # Eventually want to implement this; need to work with tranducer data
#                        QV = TRUE, clean = TRUE){
#
...
#     stations_dwr = as.data.frame(content(stations_dwr_dl, "parsed" ))
#
...
#     wl_obs_dwr_all = as.data.frame(content(wl_obs_dwr_dl, "parsed"))
#
...
#     perf_dwr_all = as.data.frame(content(perf_dwr_dl, "parsed"))
#
#
#     # Clip obs and stations to Siskiyou County
#     stations_dwr_sp = stations_dwr #copy table to make spatial object
#     coordinates(stations_dwr_sp) = ~LONGITUDE+LATITUDE #Make wells table spatial
#     proj4string(stations_dwr_sp) <- CRS("+init=epsg:4326") #assign WGS84 projection to coordinates
#     stations_dwr_sp = spTransform(stations_dwr_sp, crs(county))
#     stations_siskiyou = stations_dwr_sp[county,]
#
#     #Subset tables based on spatial clip
#     stations_dwr = stations_dwr[stations_dwr$STN_ID %in% stations_siskiyou$STN_ID,]
#     wl_obs_dwr = wl_obs_dwr_all[wl_obs_dwr_all$SITE_CODE %in% stations_siskiyou$SITE_CODE,]
#     perf_dwr = perf_dwr_all[perf_dwr_all$SITE_CODE %in% stations_siskiyou$SITE_CODE,]
#
#     # Clean up column names and add source info, and add some columns
#     colnames(wl_obs_dwr) =  clean_colnames(colnames(wl_obs_dwr))
#     wl_obs_dwr$data_table_source = "DWR_Periodic_GWL"
#
#     colnames(stations_dwr) = clean_colnames(colnames(stations_dwr))
...
#
#     #2b) VMP wells info: clean, attach to DWR  -------------------------------------------
#
...
#     irr_wells = readOGR(dsn = file.path(data_dir, "Wells and water levels"), layer = "irrigation_wells_svihm_2018")
#     mon_wells = readOGR(dsn = file.path(data_dir, "Wells and water levels"), layer = "monitoring_wells_svihm_2018")
#
#     #add coordinates to wells table
#     irr_wells = spTransform(irr_wells, crs("+init=epsg:4326"))
#     mon_wells = spTransform(mon_wells, crs("+init=epsg:4326"))
#     irr_wells$longitude = irr_wells@coords[,1]; irr_wells$latitude = irr_wells@coords[,2]
#     mon_wells$longitude = mon_wells@coords[,1]; mon_wells$latitude = mon_wells@coords[,2]
#
...
#     vmp_wells = raster::union(irr_wells, mon_wells)
...
#     stations_vmp$well_loc_source[stations_vmp$well_name %in% new_names] = "RCD_Pumping_Test" #Combine vmp and dwr
#
#
...
#     lwa_all <- lapply(l$data[tables_with_ID], compute_dtw,
#                       l$baro_df, calib$data, calib$field, is_bubbler) %>%
#       bind_rows()
#
...
#
#
#     # pull in DEM, spatialize stations, get RP
#     # wsh_10km = buffer(watershed, 1e4)
#     # dem= pgGetRast(siskiyou_spatial, "usgs_ned_n42w122_124_1arcsec_wgs84", rast = "rast", boundary = wsh_10km)
#     # dem= pgGetRast(siskiyou_spatial, "usgs_dem_1arcsec", rast = "rast", boundary = wsh_10km)
#
#     qv_wells = latlong
#     coordinates(qv_wells) = ~Lon_decimal + Lat_decimal
#     proj4string(qv_wells) <- CRS("+init=epsg:4326")
#     qv_wells = spTransform(qv_wells, crs(dem_watershed))
#     qv_wells$ground_elev_mamsl = raster::extract(x = dem_watershed, y = qv_wells)
#
...
#   # copy_to(dest = siskiyou_tables, df = wl_obs,
#   #         name = "wl_observations", overwrite=TRUE,
#   #         temporary = FALSE, indexes = list("stn_id", "well_code", "date"))
#   # copy_to(dest = siskiyou_tables, df = wells,
#   #         name = "wl_data_wells",overwrite=TRUE,
#   #         temporary = FALSE, indexes = list( "stn_id","well_code"))
#   # copy_to(dest = siskiyou_tables, df = perf,
#   #         name = "wl_data_perf", overwrite = TRUE,
#   #         temporary = FALSE, indexes = list ("stn_id", "well_code"))
#
#   # wells_spatial = wells
#   # coordinates(wells_spatial) = ~longitude + latitude #Make the point-perforations table spatial
#   # proj4string(wells_spatial) <- CRS("+init=epsg:4326") #assign WGS84 projection to coordinates
#   # wells_spatial = spTransform(wells_spatial, crs("+init=epsg:3310"))#assign 3310 projection to coordinates
#   # pgInsert(conn = siskiyou_spatial, name = "wl_data_wells_sp", data.obj = wells_spatial, df.mode = FALSE, overwrite = TRUE) # Make a spatial object of the well locations
#
# }
#
#
...
# # NOTE: If VMP =TRUE, currently (Sep 2019) only CMK can run it, since she's got the VMP data on her local drive.
# wl_info = get_wl_data(VMP = TRUE, DWR = TRUE, PSI = FALSE, LWA_TD = FALSE, QV = TRUE, clean = TRUE)
# wl_obs = wl_info[[1]]
# wells = wl_info[[2]]
# perf = wl_info[[3]]
#
#
# # #call function to perform the web scraping and upload
...
#     wsh_10km = buffer(wsh, 1e4)
#     # already made dem_watershed
#     # dem= pgGetRast(siskiyou_spatial, "usgs_ned_n42w122_124_1arcsec_wgs84", rast = "rast", boundary = wsh_10km)
#     # dem= pgGetRast(siskiyou_spatial, "usgs_dem_1arcsec", rast = "rast", boundary = wsh_10km)
#     # dem_watershed=crop(dem,wsh)
#   }
#
#   # 1) DWR wl, wells and perf data: webscrape  -----------------------------------------------
#   if(DWR == TRUE){
...
#     # Home made text parser
#
#     wl_obs_dwr_text = content(wl_obs_dwr_dl, as = "text")
#     wl_obs_dwr_all = dwr_text_parser(dwr_text = wl_obs_dwr_text)
#     # wl_obs_dwr_lines = unlist(strsplit(wl_obs_dwr_text, split = "\n"))
#     #
#     # colnames_wl_obs_dwr = unlist(strsplit(wl_obs_dwr_lines[1], ","))
#     # ncol_wl_obs_dwr = length(colnames_wl_obs_dwr)
#     # nrow_wl_obs_dwr = length(wl_obs_dwr_lines) - 1
#     #
#     # wl_obs_dwr_data = strsplit(x = wl_obs_dwr_lines[2:nrow_wl_obs_dwr], split = ",")
#     # weird_ones = which(lengths(wl_obs_dwr_data)<14)
#     # wl_obs_dwr_data_14 = wl_obs_dwr_data[lengths(wl_obs_dwr_data)==14]
#     # wl_obs_dwr_data_other = wl_obs_dwr_data[lengths(wl_obs_dwr_data)>14]
#     #
#     # wl_obs_dwr_1 = matrix(data = unlist(wl_obs_dwr_data_14),
#     #                     nrow = length(wl_obs_dwr_data_14),
#     #                     ncol = ncol_wl_obs_dwr, byrow = T)
#     # #make a secondary matrix of the data from the ones with commas in the comments
#     # wl_obs_dwr_2 = matrix(data = NA, nrow = length(wl_obs_dwr_data_other),
#     #                       ncol = ncol_wl_obs_dwr)
#     #
#     # for(i in 1:length(wl_obs_dwr_data_other)){
#     #   split_data = unlist(wl_obs_dwr_data_other[[i]])
#     #   comments = paste(split_data[14:length(split_data)], collapse = ",")
#     #   wl_obs_dwr_2[i,1:13] = split_data[1:13]
#     #   wl_obs_dwr_2[i,14] = comments
#     # }
#     #  wl_obs_dwr_matrix = rbind(wl_obs_dwr_1, wl_obs_dwr_2)
#     #  wl_obs_dwr_all = as.data.frame(wl_obs_dwr_matrix)
#     #  colnames(wl_obs_dwr_all) = colnames_wl_obs_dwr
#     #  colnames(wl_obs_dwr_all)[colnames(wl_obs_dwr_all)=="msmt_cmt\r"] = "msmt_cmt"
#     #  wl_obs_dwr_all$msmt_cmt = gsub(pattern = "\r", replacement = "",
#     #                                x = wl_obs_dwr_all$msmt_cmt)
#
...
#     # county = get_postgis_query(siskiyou_spatial, "SELECT * FROM siskiyou_county_boundary", geom_name = "geom")
#     stations_dwr_sp = stations_dwr_all #copy table to make spatial object
#     coordinates(stations_dwr_sp) = ~longitude+latitude #Make wells table spatial
#     proj4string(stations_dwr_sp) <- CRS("+init=epsg:4326") #assign WGS84 projection to coordinates
#     stations_dwr_sp = spTransform(stations_dwr_sp, crs(county))
#     stations_siskiyou = stations_dwr_sp[county,]
...
#     coordinates(rp_missing_sp)= ~longitude + latitude
#     proj4string(rp_missing_sp) = CRS("+init=epsg:4326")
#     rp_missing_sp = spTransform(rp_missing_sp, CRSobj = crs(dem_watershed))
#     rp_missing_sp$rp_elev_m = raster::extract(x = dem_watershed, y = rp_missing_sp)
#
...
#     missing_ref_pt_df$rp_elev_ft[!is.finite(missing_ref_pt_df$rp_elev_ft)] =
#       3.28084 * rp_missing_sp$rp_elev_m[match(missing_ref_pt_df$well_code[!is.finite(missing_ref_pt_df$rp_elev_ft)],
...
# # Get an updated table of water levels and associated stations
# # NOTE: If VMP =TRUE, currently (Sep 2019) only CMK can run it, since she's got the VMP data on her local drive.
# wl_info = get_wl_data(VMP = TRUE, DWR = TRUE, PSI = FALSE, LWA_TD = FALSE,
#                       QV = TRUE, clean = TRUE)
# wl_obs = wl_info[[1]]
# wells = wl_info[[2]]
# perf = wl_info[[3]]
...
#
# # CURRENTLY HERE
# # wl_info_test = get_wl_data(VMP = TRUE, DWR = TRUE, PSI = FALSE, LWA_TD = TRUE,
# #                            QV = TRUE, clean = TRUE)
#
#
# # #call function to perform the web scraping and upload
...
# # cdec_sp = spTransform(cdec_sp, crs("+init=epsg:3310"))
# #
# # plot(watershed)
...
# So, I will read them in from local. below.
...
# scott_stations = cdec_stations(river_basin = "scott r") #can also call using county = "siskiyou"
#
# snow_stns = unique(scott_stations$station_id)
# snow_stn_sp = cdec_stations(river_basin = "scott r")
# snow_stn_sp = snow_stn_sp[snow_stn_sp$station_id %in% tolower(snow_stns),]
# coordinates(snow_stn_sp) =  ~longitude + latitude
# proj4string(snow_stn_sp) <- CRS("+init=epsg:4326")
# snow_stn_sp = spTransform(snow_stn_sp,  crs("+init=epsg:3310"))
...
# # Fish data ---------------------------------------------------------------
#
# spawners = read.csv(file.path(data_dir,"cdfw_2021a_tab5_Klamath and Scott Chinook Natural Spawner escapment.csv"))
# smolt_per_fem = read.csv(file.path(data_dir, "cdfw_2021a_tab7_Coho_smolt_production_per_female_2008-2018.csv"))
# chinook_abun = read.csv(file.path(data_dir,"cdfw_2021a_tab4_Chinook abundance estimates 2008-2020.csv"))
# chinook_spawn_and_juv = read.csv(file.path(data_dir,"massie_2020_Scott River Chinook Adult_juv_data_BY_1999_2020.csv"))
# coho_abun = read.csv(file.path(data_dir,"cdfw_2021a_fig18_Coho abundance estimates 2007-2020.csv"))
# redds = read.csv(file.path(data_dir, "rcd_2020_coho_spawning_surveys.csv"))
# outmigs = read.csv(file.path(data_dir,"cdfw_2021a_tab6_Coho smolt outmigrant survival 2004-2017.csv"))
...
# coordinates(ground_obs_pts) = ~Longitude_pt1 + Latitude_pt1
# proj4string(ground_obs_pts) <- CRS("+init=epsg:4326") #assign WGS84 projection to coordinates
# ground_obs_pts = spTransform(ground_obs_pts, crs("+init=epsg:3310"))
#
# ### 2. Draft chokepoints and salmon reach access, derived from above obs.
# hab_access = readOGR(file.path(connect_dir, "habitat access by flow_draft_2022.01.12.shp"))
# chokepts = readOGR(file.path(connect_dir, "Scott River habitat access chokepoints_draft_2022.01.12.shp"))
#
...
# sat_pts = readOGR(file.path(connect_dir,
#                             "Streamflow connectivity points - UPDATED.kml"),
#                   "Streamflow connectivity points")
# sat_pts = spTransform(sat_pts, crs("+init=epsg:3310"))
# # classify
# sat_pts@data$Description[grepl(pattern = "Sentinel", x = sat_pts@data$Name)]="Sentinel"
...
# sat_pts@data$Description[grepl(pattern = "Sentinel", x = sat_pts@data$Name)]="Sentinel"
# sat_pts@data$Description[sat_pts@data$Name %in% reach_info_pts]="Reach Info"
# sat_pts@data$Description[is.na(sat_pts@data$Description)] = "Google Earth"
